...
async function mockRandomOrg(page: Page, values: number[]): Promise<void> {
  let cursor = 0;
...
async function disableCryptoFallback(page: Page): Promise<void> {
...
test.describe('Language toggle', () => {
  ...
  test('lang flag shows opposite language', async ({ page }) => {
    await page.addInitScript(() => localStorage.clear());
    await gotoApp(page);
    await expect(page.locator('#lang-flag')).toHaveAttribute('src', /gb/);

    await page.click('#lang-toggle');
    await expect(page.locator('#lang-flag')).toHaveAttribute('src', /fr/);
  });
...
test.describe('Clear button', () => {
  ...
  test('hides error banner after clearing', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      if (!formulaInput || !rollButton) return;
  ...
  test('error banner is hidden after a successful retry', async ({ page }) => {
    ...
    await page.route(/random\.org\/integers/, async route => {
      ...
      if (callCount === 1) {
        ...
        await route.fulfill({ status: 200, contentType: 'text/plain', body: '4\n' });
      }
    });
    await gotoApp(page);
    await page.fill('#formula-input', '1d6');
