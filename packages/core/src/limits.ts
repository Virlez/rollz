import {
  DEFAULT_MAX_DICE_PER_FORMULA,
  DEFAULT_MAX_FORMULAS_PER_REQUEST,
...
export function resolveLimits(overrides: Partial<RollLimits> = {}): RollLimits {
  return {
...
export function validateRollRequest(request: RollRequest, limits: Partial<RollLimits> = {}): RollRequest {
...
export function parseAndValidateRollRequest(rawInput: string, limits: Partial<RollLimits> = {}): RollRequest {
  const resolved = resolveLimits(limits);
