import type { DrawNumbers, RollResult, Token, TokenResult } from './types.js';

function sumValues(values: number[]): number {
  return values.reduce((left, right) => left + right, 0);
}

function mapSuccesses(values: number[], predicate: (value: number) => boolean): boolean[] {
  return values.map(value => predicate(value));
}

function createTokenResults(tokens: Token[]): Array<TokenResult | null> {
...
function hasInlineAdvancedTokens(tokens: Token[]): boolean {
  return tokens.some(token => token.type === 'dice' && (token.successThreshold !== undefined || token.rerollAtOrBelow !== undefined));
}

function allDiceUseThreshold(tokens: Token[]): boolean {
  const diceTokens = tokens.filter((token): token is Extract<Token, { type: 'dice' }> => token.type === 'dice');
  return diceTokens.length > 0 && diceTokens.every(token => token.successThreshold !== undefined);
}

async function evaluateInlineAdvancedTokens(tokens: Token[], drawNumbers: DrawNumbers): Promise<RollResult> {
  let total = 0;
  ...
  for (const [index, token] of tokens.entries()) {
    ...
    const successThreshold = token.successThreshold;
    const successMatches = successThreshold !== undefined
      ? mapSuccesses(finalRolls, value => value >= successThreshold)
      : [];
    const successCount = successMatches.filter(Boolean).length;
    const subtotal = successThreshold !== undefined ? successCount : sumValues(finalRolls);
...
export async function evaluateTokens(
  ...
  const diceNeeded: Record<number, number> = {};
  ...
  const pools: Record<number, { numbers: number[]; cursor: number }> = {};
  ...
  for (let i = 0; i < tokens.length; i += 1) {
    ...
    if (token.type === 'dice') {
      ...
      if (isAdvGroup) {
        ...
        let keptFirst: number;
        let discFirst: number;
