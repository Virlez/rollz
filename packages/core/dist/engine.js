...
export async function evaluateTokens(tokens, options) {
    ...
    if (successMode) {
        ...
        tokens.forEach((token, index) => {
            if (token.type !== 'dice' || index === firstDiceIndex)
                return;
            tokenResults[index] = {
                finalRolls: [],
                originalRolls: [],
    ...
    const fetched = await Promise.all(Object.entries(diceNeeded).map(([sides, count]) => drawNumbers(count, Number(sides)).then(numbers => ({
        sides: Number(sides),
        numbers,
    }))));
    const pools = {};
